...
function fetchAvailableMovies() {
  ...
  async function processBooking(userId) {
    ...
    const [fetchedmovie,userdetails]=await Promise.all([
        fetchAvailableMovies(),
        checkUserDetails(userId),
        ])
        console.log(userdetails);
    // Sequential Activities: Choose movie and select showtime
    movieList=["Movie A", "Movie B", "Movie C", "Movie D"]
      const choosen=await chooseMovie(movieList);
      console.log(choosen);
      const movie=await selectShowtime(movieList[0]);
      console.log(movie);
      const pay=await makePayment();
      console.log(pay);
    // Concurrent Activities: Send email and SMS confirmations
      const [mailed,sms]=await Promise.all([
